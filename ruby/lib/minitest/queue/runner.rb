    ...
    class Runner
      ...
      def bisect_command
        ...
        if run_tests_in_fork(failing_order)
          ...
          enhanced_log = failing_order.to_a.map do |test|
            source_location = test.respond_to?(:source_location) ? test.source_location : nil
            file_path = source_location&.first || 'unknown'
            line_number = source_location&.last || 0
            "#{test.id} #{file_path}:#{line_number}"
          end
          
          File.write('log/bisect_test_details.log', enhanced_log.join("\n"))
