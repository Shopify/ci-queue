...
module ReporterTestHelper
  ...
  def result(name, **kwargs)
    ...
    result.queue_entry = CI::Queue::QueueEntry.format("#{result.klass}##{result.name}", "#{Minitest::Queue.project_root}/test/my_test.rb") if result.respond_to?(:queue_entry=)
    result
  end
  ...
  def runnable(name, failure: nil, requeued: false, skipped: false, unexpected_error: false)
    ...
    # Push Requeue before Skip when both set so requeued? is true (recorder checks requeued? before success/skip)
