...
module CI::Queue
  class ConfigurationTest < Minitest::Test
    ...
    def test_namespace
      argv = ['--build', '9e08ef3c-d6e6-4a86-91dd-577ce5205b8e']
      config = Configuration.new({}, argv)
      assert_equal '9e08ef3c-d6e6-4a86-91dd-577ce5205b8e', config.build_id

      argv = ['--build', '9e08ef3c-d6e6-4a86-91dd-577ce5205b8e', '--namespace', 'browser']
      config = Configuration.new({}, argv)
    ...
    def test_parses_file_correctly
      Tempfile.open('flaky_test_file') do |file|
        ...
        env = { 'CI_QUEUE_FLAKY_TESTS' => file.path }
        config = Configuration.new(env, [])
        flaky_tests = config.flaky_tests
        SharedTestCases::TEST_NAMES.each do |test|
          assert_includes flaky_tests, test
        end
      end

      env = { 'CI_QUEUE_FLAKY_TESTS' => '/tmp/does-not-exist' }
      flaky_tests = Configuration.new(env, []).flaky_tests
