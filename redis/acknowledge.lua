...
local worker_queue_key = KEYS[6]

local entry = ARGV[1]
local error = ARGV[2]
local ttl = ARGV[3]

-- Only the current owner can remove the entry from the running set.
-- If ownership was transferred (e.g. via reserve_lost), the stale worker
-- must not remove the running entry — that would let the supervisor think
-- the queue is exhausted while the new owner is still processing.
